...
export default function DataExplorerSection({
  ...
  const makes = [
    "all",
    ...Array.from(new Set(data.map((item) => item.Veh_Make_ID))),
  ];
    ...
    selectedMake !== "all"
      ? [
          ...
          ...Array.from(
            new Set(
              data
                .filter((item) => item.Veh_Make_ID === selectedMake)
                .map((item) => item.Veh_Mod_ID)
            )
          ),
        ]
      : ["all"];
  ...
  const modelYears = [
    "all",
    ...Array.from(
      new Set(data.map((item) => item.Veh_Mod_Year.toString()))
    ).sort(),
  ];
  const crashYears = [
    "all",
    ...Array.from(
      new Set(data.map((item) => item.CrashYear.toString()))
    ).sort(),
  ];
  ...
  const filteredData = data.filter((item) => {
    const matchesMake =
      selectedMake === "all" ? true : item.Veh_Make_ID === selectedMake;
    const matchesModel =
      selectedModel === "all" ? true : item.Veh_Mod_ID === selectedModel;
    const matchesModelYear =
      selectedModelYear === "all"
        ? true
        : item.Veh_Mod_Year.toString() === selectedModelYear;
    const matchesCrashYear =
      selectedCrashYear === "all"
        ? true
        : item.CrashYear.toString() === selectedCrashYear;
    const matchesSearch =
      searchQuery === ""
        ? true
  ...
  const handleSort = (field: string) => {
    if (sortField === field) {
      ...
      setSortField(field as keyof CrashData);
  ...
  return (
    <section
      ref={sectionRef}
      className="min-h-screen flex flex-col items-center justify-center px-4 py-20 bg-slate-50"
    >
      <div className="max-w-6xl mx-auto w-full">
        ...
        >
          ...
          <p className="text-lg md:text-xl text-slate-600 max-w-3xl mx-auto text-center">
            Dive deeper into the Texas vehicle crash dataset. Filter, sort, and
            analyze the data to discover specific patterns and insights that
            matter to you.
          </p>
        </motion.div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={setSelectedModelYear}
                  >
                    <SelectTrigger className="bg-white">
                ...
                <div>
                  ...
                  <Select
                    value={selectedCrashYear}
                    onValueChange={setSelectedCrashYear}
                  >
                    <SelectTrigger className="bg-white">
